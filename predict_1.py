...
# 	y = tf.placeholder(tf.float32, [None, 3], name = 'y')
	...
	b = graph.get_tensor_by_name("bias1:0")

	y_ = tf.get_collection("first_layer")[0]
		
	W_p = graph.get_tensor_by_name("weight2:0")
	b_p = graph.get_tensor_by_name("bias2:0")
...
# 	y_p = tf.matmul(y_, W_p) + b_p
	...
	for prot in prot_names:
		next_prot = InputParser(prot_dir, prot)
		...
		for i in range(len(next_prot.getWindows())):
			next_window = n.reshape(next_prot.getWindows()[i], (1, 300)).astype(float)
